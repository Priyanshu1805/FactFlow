...
export async function runMemeContentTracker() {
  ...
  try {
    ...
    for (const url of MEME_SOURCES) {
      try {
        ...
        const memes = response.data.data.children
          .filter((c: any) => !c.data.is_video && c.data.url?.endsWith(".jpg")) // Get image memes only
          .map((child: any) => ({
    ...
    for (const meme of processedMemes) {
      if (!meme.headline || !meme.content) continue
      
      const rawMeme = rawMemes[i] || rawMemes[0]
      const existing = await NewsArticle.findOne({ title: meme.headline })
      if (!existing) {
