...
export async function runAiNewsAggregator() {
  ...
  try {
    ...
    for (const url of REDDIT_SOURCES) {
      try {
        ...
        rawItems.push(...response.data.data.children.map((child: any) => ({
    ...
    if (usedFallback) {
      processedNews = rawItems.map((item: any) => ({
        headline: item.title,
    ...
    for (const news of processedNews) {
      if (!news.headline) continue
      const existing = await NewsArticle.findOne({ title: news.headline })
      if (!existing) {
