...
function fallbackClassifier(title: string, description: string, defaultCat: string): string {
  const text = (title + " " + description).toLowerCase()
  
  if (/\b(cricket|football|ipl|fifa|tennis|nba|olympics|kohli|messi)\b/.test(text)) return "Sports"
  if (/\b(ai|iphone|android|startup|coding|software|gadget|google|microsoft|apple)\b/.test(text)) return "Tech"
  if (/\b(bjp|congress|pm modi|election|parliament|government|minister|biden|trump)\b/.test(text)) return "Politics"
  if (/\b(meme|funny|viral|lol|rofl|reddit)\b/.test(text)) return "Memes"
  if (/\b(health|fitness|fashion|travel|food|wellness|beauty)\b/.test(text)) return "Lifestyle"
  if (/\b(breaking|just in|live)\b/.test(text)) return "Trending"
...
function calculateQualityScore(item: any): number {
  ...
  if (item.source && topSources.some(s => item.source.toLowerCase().includes(s))) score += 20
  else if (item.type === "reddit") score += 5 // user generated
  else if (item.type === "nitter") score += 10

  // 2. Engagement (for reddit/social)
  if (item.score && item.score > 1000) score += 15
  else if (item.score && item.score > 100) score += 5

  // 3. Media presence
  if (item.imageUrl && item.imageUrl.length > 5) score += 10

  // 4. Recency (within last 2 hours)
  const ageInHours = (Date.now() - new Date(item.publishedAt).getTime()) / (1000 * 60 * 60)
  if (ageInHours <= 2) score += 10

  return Math.min(score, 100) // Max 100
}

export async function smartProcessor(rawItems: any[], existingTitles: string[] = []) {
  ...
  const processedItems = uniqueItems.map((item, idx) => {
    ...
    const detectedLang = detectLanguage(title).toLowerCase()
