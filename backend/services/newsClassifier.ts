...
export function keywordClassify(title: string, description: string): string {
  ...
  for (const [section, keywords] of Object.entries(KEYWORD_MAP)) {
    ...
    for (const kw of keywords) {
      ...
      if (t.includes(lowerKw)) score += 2
      if (d.includes(lowerKw)) score += 1
    }
...
export async function classifyWithAI(title: string, description: string): Promise<string> {
  ...
  try {
    ...
    await setCached(cacheKey, section, 3600) // 1 hour cache
    return section
  } catch (error) {
...
export async function classifyArticle(title: string, description: string): Promise<string> {
  try {
