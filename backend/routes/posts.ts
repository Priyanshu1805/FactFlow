...
// ─────────────────────────────────────────────
router.post("/create", upload.array("media", 10), async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      parsedHashtags = typeof hashtags === "string" ? hashtags.split(",").map((t: string) => t.trim()) : []
    }
  ...
  try {
    ...
    if (firebaseUid) {
      ...
      if (user) {
        ...
        // Include self
        followingIds.push(user._id as any);
      ...
      if (user && user.settings?.content) {
        ...
        if (hiddenPublishers && hiddenPublishers.length > 0) {
          posts = posts.filter(post => {
            ...
            return !hiddenPublishers.some((p: string) => 
              author.name?.toLowerCase().includes(p.toLowerCase()) || 
...
router.post("/:id/like", async (req: Request, res: Response) => {
  try {
    ...
    const hasLiked = post.likes.includes(user._id as any)
  ...
  try {
    ...
    if (exists) {
      user.savedItems = user.savedItems.filter(
        (item) => !(item.itemId === post._id.toString() && item.itemType === "social_post")
      ) as any
...
router.post("/:id/comment", async (req: Request, res: Response) => {
  try {
    ...
    if (!firebaseUid || !text) return res.status(400).json({ error: "firebaseUid and text required" })
    ...
    const post = await Post.findById(req.params.id)
    if (!post) return res.status(404).json({ error: "Post not found" })
    if (post.commentsDisabled) return res.status(403).json({ error: "Comments are disabled for this post" })
...
// ─────────────────────────────────────────────
router.put("/:id/comment/:commentId/action", async (req: Request, res: Response) => {
  try {
    const { firebaseUid, actionType } = req.body
    if (!firebaseUid || !actionType) return res.status(400).json({ error: "firebaseUid and actionType required" })
    ...
    if (actionType === "like") {
      const hasLiked = comment.reactions.like.includes(user._id as any)
      if (hasLiked) {
        comment.reactions.like = comment.reactions.like.filter((id) => id.toString() !== user._id.toString())
      } else {
        comment.reactions.like.push(user._id as any)
      }
