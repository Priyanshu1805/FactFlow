...
router.get("/profile", async (req: Request, res: Response) => {
  try {
    const { firebaseUid, email } = req.query as { firebaseUid: string; email: string }
    if (!firebaseUid && !email) {
      return res.status(400).json({ error: "firebaseUid or email required" })
    }

    let user = await User.findOne(firebaseUid ? { firebaseUid } : { email })

    // Auto-create profile if first login (Firebase user, not in MongoDB yet)
    if (!user && email) {
      const name = req.query.name || "Fact Flow User"
      const defaultAvatar = `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(name as string)}`
...
// Get user settings
// ─────────────────────────────────────────────
router.get("/settings/:firebaseUid", async (req: Request, res: Response) => {
  try {
...
router.put("/settings", async (req: Request, res: Response) => {
  try {
    ...
    if (!firebaseUid || !settings) return res.status(400).json({ error: "firebaseUid and settings required" })
...
router.post("/save", async (req: Request, res: Response) => {
  try {
    ...
    if (!firebaseUid || !itemId || !itemType) return res.status(400).json({ error: "Missing parameters" })
...
// ─────────────────────────────────────────────
router.post("/unsave", async (req: Request, res: Response) => {
  try {
    const { firebaseUid, itemId, itemType } = req.body
    if (!firebaseUid || !itemId || !itemType) return res.status(400).json({ error: "Missing parameters" })
...
// ─────────────────────────────────────────────
router.get("/saved/:firebaseUid", async (req: Request, res: Response) => {
  try {
    const { firebaseUid } = req.params
    const user = await User.findOne({ firebaseUid })
    if (!user) return res.status(404).json({ error: "User not found" })
    ...
    const populatedPosts = await NewsArticle.find({ _id: { $in: postIds } })
      .select("title image category createdAt excerpt source")
    
    const populatedShorts = await Reel.find({ _id: { $in: shortIds } })
      .select("title thumbnailUrl views source youtubeId")

    const populatedSocialPosts = await Post.find({ _id: { $in: socialPostIds } })
...
router.get("/public/:username", async (req: Request, res: Response) => {
  try {
    ...
    const viewerUid = req.query.viewerUid as string
    let isFollowing = false
    if (viewerUid) {
      const viewerUser = await User.findOne({ firebaseUid: viewerUid })
      if (viewerUser) {
...
router.post("/:id/follow", async (req: Request, res: Response) => {
  try {
    ...
    if (!viewerUid) return res.status(400).json({ error: "Viewer UID required" });

    const followerUser = await User.findOne({ firebaseUid: viewerUid });
    ...
    if (!followingUser) return res.status(404).json({ error: "Target user not found" });
...
// Body: { viewerUid: string }
// ─────────────────────────────────────────────
router.post("/:id/toggle-follow", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.post("/avatar", upload.single("avatar"), async (req: Request, res: Response) => {
  try {
    ...
    // Save to MongoDB
    await User.findOneAndUpdate({ firebaseUid }, { $set: { avatar: avatarUrl } })
...
// ─────────────────────────────────────────────
router.post("/schedule-deletion", async (req: Request, res: Response) => {
  try {
...
router.post("/2fa/verify", async (req: Request, res: Response) => {
  try {
    ...
    if (!firebaseUid || !token) return res.status(400).json({ error: "firebaseUid and token required" })

    const user = await User.findOne({ firebaseUid })
    if (!user || !user.twoFactorSecret) return res.status(400).json({ error: "2FA setup not initiated" })
...
// ─────────────────────────────────────────────
router.get("/search-mentions", async (req: Request, res: Response) => {
  try {
    const { q } = req.query
...
router.post("/post", async (req: Request, res: Response) => {
  try {
    ...
    if (!firebaseUid || !mediaUrl) return res.status(400).json({ error: "Missing required fields" })
...
router.post("/follow", async (req: Request, res: Response) => {
  try {
    ...
    if (!firebaseUid || !targetUsername) return res.status(400).json({ error: "firebaseUid and targetUsername required" })
...
router.post("/history", async (req: Request, res: Response) => {
  try {
    ...
    if (!firebaseUid || !itemId || !itemType) return res.status(400).json({ error: "Missing required fields" })
...
router.get("/history", async (req: Request, res: Response) => {
  try {
    ...
    const historyItems = await History.find({ user: user._id })
      .sort({ timestamp: -1 })
      .limit(50)
      .lean()
    ...
    const enrichedHistory = await Promise.all(historyItems.map(async (item) => {
      ...
      if (item.itemType === "post") {
        details = await NewsArticle.findById(item.itemId).select("title image category createdAt").lean()
      } else if (item.itemType === "reel") {
        details = await Reel.findById(item.itemId).select("title thumbnailUrl views").lean()
      }
      return {
        ...item,
        details
...
router.delete("/history", async (req: Request, res: Response) => {
  try {
    const firebaseUid = (req.query.firebaseUid || req.body.firebaseUid) as string
    if (!firebaseUid) return res.status(400).json({ error: "firebaseUid is required" })
...
router.get("/export-data", async (req: Request, res: Response) => {
  try {
    ...
    const history = await History.find({ user: user._id }).sort({ timestamp: -1 }).lean()
    const comments = await Comment.find({ authorId: user._id }).sort({ createdAt: -1 }).lean()
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    if (viewerUid) {
      viewerUser = await User.findOne({ firebaseUid: viewerUid }).lean()
    }
...
// Body: { adminFirebaseUid }
// ─────────────────────────────────────────────
router.put("/:id/toggle-ban", async (req: Request, res: Response) => {
  try {
...
// Body: { adminFirebaseUid }
// ─────────────────────────────────────────────
router.put("/:id/toggle-admin", async (req: Request, res: Response) => {
  try {
    const { adminFirebaseUid } = req.body
    const adminUser = await User.findOne({ firebaseUid: adminFirebaseUid })
