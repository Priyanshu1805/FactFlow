...
router.get("/stats", async (req: Request, res: Response) => {
  ...
  try {
    const article = await ExternalArticle.findOne({ url: String(url) })
...
router.post("/interact", async (req: Request, res: Response) => {
  ...
  try {
    let updateDoc: any = {}
    if (action === "like") {
      if (!userId) return res.status(400).json({ error: "User ID required for liking" })
      if (value === 1) {
      ...
      if (!userId) return res.status(400).json({ error: "User ID required for disliking" })
      if (value === 1) {
      ...
      if (!userId) return res.status(400).json({ error: "User ID required for saving" })
      if (value === 1) {
    ...
    const article = await ExternalArticle.findOneAndUpdate(
      { url },
      updateDoc,
