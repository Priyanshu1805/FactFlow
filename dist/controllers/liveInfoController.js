...
const getLiveInfo = async (req, res) => {
    try {
        ...
        const promises = widgets.map(async (widget) => {
            try {
                switch (widget) {
                    case "weather":
                        if (lat && lon) {
                            const weatherRes = await axios_1.default.get(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code,wind_speed_10m&daily=sunrise,sunset&timezone=auto`);
                            const aqiRes = await axios_1.default.get(`https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${lat}&longitude=${lon}&current=us_aqi`);
                            results.weather = {
                    ...
                    case "finance":
                        ...
                        const quotes = await yahoo_finance2_1.default.quote(symbols);
                        const financeData = {};
                        quotes.forEach((q) => {
...
//# sourceMappingURL=liveInfoController.js.map