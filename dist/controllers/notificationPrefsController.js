...
exports.testEmailDigest = testEmailDigest;
...
async function getNotificationPrefs(req, res) {
    try {
        ...
        const prefs = await NotificationPrefs_1.NotificationPrefs.findOne({ userId });
        if (!prefs) {
            // Default preferences
            res.json({
                success: true,
...
async function updateNotificationPrefs(req, res) {
    try {
        ...
        const prefs = await NotificationPrefs_1.NotificationPrefs.findOneAndUpdate({ userId }, { $set: req.body }, { upsert: true, new: true });
        res.json({ success: true, data: prefs });
    }
    catch (err) {
        res.status(500).json({ success: false, error: err.message });
...
async function savePushSubscription(req, res) {
    try {
        ...
        const prefs = await NotificationPrefs_1.NotificationPrefs.findOneAndUpdate({ userId }, { $set: { pushEnabled: true, pushSubscription: subscription } }, { upsert: true, new: true });
...
async function testNotifications(req, res) {
    try {
        ...
        const prefs = await NotificationPrefs_1.NotificationPrefs.findOne({ userId }).lean();
...
async function testEmailDigest(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ success: false, error: "Not authenticated" });
        ...
        const topArticles = await NewsArticle_1.NewsArticle.find({})
            .sort({ publishedAt: -1 })
            .limit(3)
            .lean();
...
//# sourceMappingURL=notificationPrefsController.js.map