...
exports.runGlobalPremiumAggregator = runGlobalPremiumAggregator;
...
async function runGlobalPremiumAggregator() {
        ...
        try {
            ...
            for (const item of (parsed.items || []).slice(0, 5)) {
                if (!item.title || item.title.length < 15)
                    continue;
                // 🧠 1. CLICKBAIT FILTER
                if ((0, smartNewsUtils_1.isClickbait)(item.title)) {
                    console.log(`🛡️  [Smart Filter] Rejected clickbait: "${item.title}"`);
                    continue;
                }
                // Fast title deduplication check
                const exists = await NewsArticle_1.NewsArticle.exists({ title: item.title });
                if (exists)
                    continue;
                ...
                const { isDuplicate, promoteToBreaking } = await (0, smartNewsUtils_1.processSmartArticle)(item.title, excerpt, feed.category);
                if (isDuplicate) {
                ...
                const smartTags = (0, smartNewsUtils_1.extractEntities)(item.title + " " + excerpt);
                ...
                if (promoteToBreaking) {
                    ...
                    try {
                        const { NotificationPrefs } = await Promise.resolve().then(() => __importStar(require("../models/NotificationPrefs")));
                        const prefs = await NotificationPrefs.find({ breakingNews: true }).lean();
                        if (prefs.length > 0) {
                            const userIds = prefs.map((p) => p.userId.toString());
                            await (0, notificationService_1.createBulkNotifications)(userIds, {
                                type: "breaking_news",
                                title: "🚨 Breaking News",
...
//# sourceMappingURL=globalPremiumAggregator.js.map