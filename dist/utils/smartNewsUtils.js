...
exports.isClickbait = isClickbait;
exports.extractEntities = extractEntities;
exports.calculateSimilarity = calculateSimilarity;
exports.processSmartArticle = processSmartArticle;
...
function isClickbait(title) {
    ...
    if (capsRatio > 0.5 && title.length > 20)
        return true;
    if ((title.match(/!/g)?.length || 0) > 2)
        return true;
 ...
 */
async function processSmartArticle(title, content, category) {
    // Check articles in the same category from the last 12 hours
    const twelveHoursAgo = new Date(Date.now() - 12 * 60 * 60 * 1000);
    const recentArticles = await NewsArticle_1.NewsArticle.find({
        category,
...
//# sourceMappingURL=smartNewsUtils.js.map