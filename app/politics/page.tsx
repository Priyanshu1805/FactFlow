...
export default function PoliticsNewsPage() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/news?category=Politics&limit=50&${useAuthStore.getState().user?.uid ? 'firebaseUid=' + useAuthStore.getState().user?.uid : ''}`)
      .then((res) => {
            ...
            {news.map((article, idx) => (
              <Link href={`/article/${article._id}`} key={article._id || idx} className="block group cursor-pointer">
                ...
                <div className="relative aspect-video overflow-hidden">
                  <Image src={article.image || "https://images.unsplash.com/photo-1529107386315-e1c731f2ca75?w=600&h=400&fit=crop"} alt={article.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
