...
import { Laugh, Clock, ArrowLeft } from "lucide-react"
...
export default function MemesPage() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/news?category=Memes&limit=50&${useAuthStore.getState().user?.uid ? 'firebaseUid=' + useAuthStore.getState().user?.uid : ''}`)
      .then((res) => res.json())
      .then((data) => {
              ...
              <Link href={`/article/${article._id}`} key={article._id || idx} className="block group cursor-pointer">
                ...
                <div className="relative aspect-video overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="px-3 py-1 bg-yellow-500 text-white text-xs font-bold rounded-md">
                      {article.tags?.[0] || article.category || "Memes"}
                    </span>
                  </div>
                </div>
                <div className="p-5">
