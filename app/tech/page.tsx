...
export default function TechNewsPage() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/news?category=Tech&limit=50&${useAuthStore.getState().user?.uid ? 'firebaseUid=' + useAuthStore.getState().user?.uid : ''}`)
      .then((res) => {
              ...
              <Link href={`/article/${article._id}`} key={article._id || idx} className="block group cursor-pointer">
                ...
                <div className="relative aspect-video overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="px-3 py-1 bg-purple-500 text-white text-xs font-bold rounded-md">
