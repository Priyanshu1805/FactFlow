...
import { Globe, ArrowRight, Clock, User, Sparkles , Volume2} from "lucide-react"
...
export function PoliticsSection() {
  ...
  const sectionNews = useMemo(() => {
    const filtered = rssItems
      .filter(item => {
        ...
        return itemSections.some(c => ["politics", "world", "government"].includes(c?.toLowerCase()));
      })
      .sort((a, b) => {
    ...
    const catImages = fallbacks["blue"] || fallbacks["green"]
    ...
    return filtered.map((item, i) => {
      ...
      tagColor: ["bg-blue-500", "bg-indigo-500", "bg-rose-500", "bg-emerald-500"][i % 4],
  ...
  return (
    <section id="politics" className={`py-12 px-4 relative overflow-hidden ${
      isDark ? "bg-transparent" : "bg-gray-50/30"
    }`}>
      <LiveNewsBanner category="Politics" />
      {isDark && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-4xl bg-blue-900/10 blur-[120px] rounded-full pointer-events-none -z-10" />
      )}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="flex items-center gap-4">
            <div className={`p-3 bg-blue-500/15 rounded-2xl backdrop-blur-xl border border-blue-500/20 shadow-[0_0_20px_rgba(var(--blue),0.2)]`}>
            ...
            <div>
              ...
              <p className={`mt-1 text-sm md:text-base font-medium ${isDark ? "text-white/60" : "text-gray-500"}`}>
                Global affairs and political updates
              </p>
            </div>
          </div>
          <Link href="/politics" className="group flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-300 bg-blue-500 hover:bg-blue-600 text-white shadow-lg shadow-blue-500/25">
            View More
        ...
        <div className={`grid transition-all duration-500 ${
          ...
          {featured && (
            ...
            >
              <Link href={`/article/${featured.id}`} className={`group relative block w-full h-full rounded-[2rem] overflow-hidden border ${isDark ? "border-white/10 hover:border-white/30" : "border-gray-200"} hover:shadow-[0_0_30px_-5px_rgba(59,130,246,0.3)] transition-all duration-700`}>
                    ...
                    <div className="flex items-center gap-4 text-white/60 text-xs md:text-sm font-semibold">
                      {displayOptions.authorName !== false && featured.authorName && (
                        <div className="flex items-center gap-2">
                          <div className={`w-6 h-6 rounded-full bg-blue-500/15 flex items-center justify-center backdrop-blur-md`}>
