...
export function LifestyleSection() {
  ...
  const sectionNews = useMemo(() => {
    const filtered = rssItems
      .filter(item => item.category === "Entertainment" || item.category === "General")
      .sort((a, b) => {
    ...
    const catImages = fallbacks["pink"] || fallbacks["green"]
    ...
    return filtered.map((item, i) => {
      ...
      tagColor: ["bg-pink-500", "bg-indigo-500", "bg-rose-500", "bg-emerald-500"][i % 4],
  ...
  return (
    <section id="lifestyle" className={`py-12 px-4 relative overflow-hidden ${
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="flex items-center gap-4">
            <div className={`p-3 bg-pink-500/15 rounded-2xl backdrop-blur-xl border border-pink-500/20 shadow-[0_0_20px_rgba(var(--pink),0.2)]`}>
        ...
        <div className={`grid transition-all duration-500 ${
          ...
          {featured && (
            ...
            >
              <Link href={`/article/${featured.id}`} className={`group relative block w-full h-full rounded-[2rem] overflow-hidden border ${isDark ? "border-white/10 hover:border-white/30" : "border-gray-200"} hover:shadow-[0_0_30px_-5px_rgba(236,72,153,0.3)] transition-all duration-700`}>
                    ...
                    <div className="flex items-center gap-4 text-white/60 text-xs md:text-sm font-semibold">
                      {displayOptions.authorName !== false && featured.authorName && (
                        <div className="flex items-center gap-2">
                          <div className={`w-6 h-6 rounded-full bg-pink-500/15 flex items-center justify-center backdrop-blur-md`}>
