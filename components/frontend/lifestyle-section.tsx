...
export function LifestyleSection() {
  ...
  const sectionNews = useMemo(() => {
    const filtered = rssItems
      ...
      .slice(0, 5) // Only 5 for home page
    ...
    return filtered.map((item, i) => ({
      ...
      image: item.image && item.image.trim() !== "" ? item.image : LIFESTYLE_FALLBACKS[i % LIFESTYLE_FALLBACKS.length],
      tag: item.category || "Lifestyle",
      tagColor: ["bg-pink-500", "bg-rose-500", "bg-fuchsia-500", "bg-purple-500"][i % 4],
  ...
  const rest = sectionNews.slice(1, 5) // 4 smaller cards

  if (sectionNews.length === 0 && !loading) return null

  return (
    <section id="lifestyle" className={`py-12 px-4 relative overflow-hidden ${isDark ? "bg-transparent" : "bg-gray-50/30"}`}>
      <LiveNewsBanner category="Lifestyle" />
      {isDark && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-4xl bg-pink-900/10 blur-[120px] rounded-full pointer-events-none -z-10" />
      )}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="flex items-center gap-4">
            <div className="p-3 bg-pink-500/15 rounded-2xl backdrop-blur-xl border border-pink-500/20">
          ...
          <Link href="/lifestyle" className={`group flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-300 bg-pink-500 hover:bg-pink-600 text-white shadow-lg shadow-pink-500/25`}>
            View More
        ...
        <div className={`grid transition-all duration-500 ${
          ...
          {featured && (
            ...
            >
              <Link href={`/article/${featured.id}`} className={`group relative block w-full h-full rounded-[2rem] overflow-hidden border ${isDark ? "border-white/10 hover:border-white/30" : "border-gray-200"} hover:shadow-[0_0_30px_-5px_rgba(236,72,153,0.4)] transition-all duration-700`}>
