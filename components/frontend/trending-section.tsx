...
export function TrendingSection() {
  ...
  const sectionNews = useMemo(() => {
    ...
    return filtered.map((item, i) => ({
      ...
      image: item.image && item.image.trim() !== "" ? item.image : TRENDING_FALLBACKS[i % TRENDING_FALLBACKS.length],
      tag: item.category || "Trending",
      tagColor: ["bg-green-500", "bg-emerald-500", "bg-teal-500", "bg-lime-600"][i % 4],
  ...
  return (
    <section id="trending" className={`py-12 px-4 relative overflow-hidden ${isDark ? "bg-transparent" : "bg-gray-50/30"}`}>
      <LiveNewsBanner category="Trending" />
      {isDark && (
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="flex items-center gap-4">
            <div className="p-3 bg-green-500/15 rounded-2xl backdrop-blur-xl border border-green-500/20">
          ...
          <Link href="/trending" className="group flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-300 bg-green-500 hover:bg-green-600 text-white shadow-lg shadow-green-500/25">
            View More
        ...
        <div className={`grid transition-all duration-500 ${
          ...
          {featured && (
            ...
            >
              <Link href={`/article/${featured.id}`} className={`group relative block w-full h-full rounded-[2rem] overflow-hidden border ${isDark ? "border-white/10 hover:border-white/30" : "border-gray-200"} hover:shadow-[0_0_30px_-5px_rgba(34,197,94,0.4)] transition-all duration-700`}>
