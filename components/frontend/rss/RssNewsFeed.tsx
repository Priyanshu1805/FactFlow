...
export function RssNewsFeed({ limit }: { limit?: number }) {
  ...
  const { lang, region } = useLanguageStore()
  const [mounted, setMounted] = useState(false)

  useEffect(() => {
    setMounted(true)
    fetchNews()
  }, [lang, region, fetchNews])
  ...
  if (loading && items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-gray-500 dark:text-zinc-400">Fetching latest news for {region} in {lang}...</p>
      </div>
    )
  }
  ...
  if (error && items.length === 0) {
    return (
      <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-6 rounded-xl text-center border border-red-100 dark:border-red-900/30">
        ...
        <button onClick={() => fetchNews()} className="flex items-center gap-2 mx-auto bg-red-100 dark:bg-red-900/40 px-4 py-2 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/60 transition-colors">
