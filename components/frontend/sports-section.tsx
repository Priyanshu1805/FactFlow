...
export function SportsSection() {
  ...
  const sectionNews = useMemo(() => {
    ...
    const catImages = fallbacks["orange"] || fallbacks["green"]
    ...
    return filtered.map((item, i) => {
      ...
      tagColor: ["bg-orange-500", "bg-indigo-500", "bg-rose-500", "bg-emerald-500"][i % 4],
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="flex items-center gap-4">
            <div className={`p-3 bg-orange-500/15 rounded-2xl backdrop-blur-xl border border-orange-500/20 shadow-[0_0_20px_rgba(var(--orange),0.2)]`}>
            ...
            <div>
              ...
              <p className={`mt-1 text-sm md:text-base font-medium ${isDark ? "text-white/60" : "text-gray-500"}`}>
                Match updates, scores, and sports news
              </p>
            </div>
          </div>
        ...
        <div className={`grid transition-all duration-500 ${
          ...
          {featured && (
            ...
            >
              <Link href={`/article/${featured.id}`} className={`group relative block w-full h-full rounded-[2rem] overflow-hidden border ${isDark ? "border-white/10 hover:border-white/30" : "border-gray-200"} hover:shadow-[0_0_30px_-5px_rgba(249,115,22,0.3)] transition-all duration-700`}>
                    ...
                    <div className="flex items-center gap-4 text-white/60 text-xs md:text-sm font-semibold">
                      {displayOptions.authorName !== false && featured.authorName && (
                        <div className="flex items-center gap-2">
                          <div className={`w-6 h-6 rounded-full bg-orange-500/15 flex items-center justify-center backdrop-blur-md`}>
